...
public class Statistic {
    ...
    private static double CalculateVolatility(List<Tick> ticks) {
        Set<String> tickInstruments = ticks.stream().map(Tick::GetInstrument).collect(Collectors.toSet());
        double volatility = 0;
        if(!tickInstruments.isEmpty()) {
        	for (String s : tickInstruments) {
                double avg = (ticks.stream().filter(t -> t.GetInstrument().equals(s))
                        .mapToDouble(Tick::GetPrice)).sum();
                double deviationSquared = ticks.stream().filter(t -> t.GetInstrument().equals(s))
                        .mapToDouble(Tick::GetPrice).map(t -> (t - avg) * (t - avg)).sum();

                volatility += deviationSquared / CalculateCount(ticks.stream().filter(t -> t.GetInstrument().equals(s)).collect(Collectors.toList()));
            }
        }
    ...
    private static double CalculateMaxDrawdown(List<Tick> ticks) {
        ...
        for (String s : tickInstruments) {
            List<Double> prices = ticks.stream().filter(t -> t.GetInstrument().equals(s)).map(Tick::GetPrice)
